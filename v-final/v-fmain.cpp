...
std::string formatToCSVSIMD(const SIMDType& disp_x, const SIMDType& disp_y, const SIMDType& disp_z,
                            const SIMDType& accel_x, const SIMDType& accel_y, const SIMDType& accel_z,
...
void partBucketForce(Particle& targetParticle,
    ...
    for (const auto& pv : particleVectors) {
        ...
        doublev twoh = targetParticle.softening + pv.softening_vec;
        gravityCalculator.calculateSplineForceSIMD<doublev>(dist_sq, twoh, potential_coeff, force_coeff);
        
        // Calculate acceleration components and potential in condensed form
        Vector3D<doublev> accel = force_coeff * pv.mass_vec * displacement;
        ...
        for (int i = 0; i < vector_width; ++i) {
            csvFile << formatToCSVSIMD(displacement.x, displacement.y, displacement.z,
                                      accel.x, accel.y, accel.z,
                                      potential_interaction, i) << "\n";
            
            // Accumulate totals using Vector3D semantics
            total_accel.x += accel.x[i];
            total_accel.y += accel.y[i];
            total_accel.z += accel.z[i];
...
int main() {
    ...
    try {
        ...
        // CSV output commented out for pure calculation benchmarking
        ...
        // Start timing for partBucketForce calculation
        // In order to test only partBucketForce, comment out the CSV output code and references to time just this process
        // Was done this way in benchmarking, runs in similar time to scalar code with CSV enabled
        ...
        // End timing for partBucketForce calculation
        auto calc_end = std::chrono::high_resolution_clock::now();
        auto calc_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(calc_end - calc_start);
        ...
        std::cout << "partBucketForce calculation time: " << calc_duration.count() << " ns (" << (calc_duration.count() / 1000000.0) << " ms)"<< std::endl;
