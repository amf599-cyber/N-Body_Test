...
## SIMD Pipeline Refactoring (v.final)
- Implemented pure SIMD pipeline from particle generation through force computation to output
- Implicit AoS↔SoA conversion via `setParticle()` and `getParticle()` methods
- Particles initialized directly in Structure of Arrays (SoA) format for vectorized processing
- No manual AoS→SoA conversion overhead in processing loops, handled implicitly through SIMD
- Maintains full double precision throughout, accepts and converts types on compilation/execution of code

## SIMD Vectorization Overhaul (v1.4) [Walked Back]
- Handled the SIMD optimization in an explicit instead of implicit manner for type handling
- Essentially scrapped this iteration of the code, instead working off edited v1.3 version
- Original version of README and Makefile and other code base will be kept in separate github folder "v1-4"
- All versions after this are to be considered final improvements on the original scope for v1 project
